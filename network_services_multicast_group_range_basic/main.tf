resource "google_compute_network" "network" {
  name                    = "test-mgr-network-${local.name_suffix}"
  auto_create_subnetworks = false
}

resource "google_network_services_multicast_domain" "multicast_domain" {
  multicast_domain_id                    = "test-mgr-domain-${local.name_suffix}"
  location = "global"
...
resource "google_network_connectivity_internal_range" "internal_range" {
  name    = "test-mgr-internal-range-${local.name_suffix}"
...
resource "google_network_services_multicast_group_range" mgr_test {
  multicast_group_range_id                   = "test-mgr-group-range-${local.name_suffix}"
  location = "global"
