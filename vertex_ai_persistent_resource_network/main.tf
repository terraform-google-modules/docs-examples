resource "google_vertex_ai_persistent_resource" "persistent_resource" {
  ...
  resource_pools {
    id = "vpr-resource-pool-${local.name_suffix}"
...
resource "google_compute_network" "vertex_network" {
  name                    = "vertex-network-${local.name_suffix}"
...
resource "google_compute_global_address" "vertex_range" {
  name          = "vertex-ip-range-${local.name_suffix}"
  purpose       = "VPC_PEERING"
...
resource "google_compute_network_attachment" "psc_attachment" {
  name                  = "psc-attachment-${local.name_suffix}"
  region                = "us-central1"
